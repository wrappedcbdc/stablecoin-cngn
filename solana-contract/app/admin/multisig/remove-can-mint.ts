// scripts/multisig/operations/remove-can-mint.ts
import { PublicKey } from "@solana/web3.js";
import * as anchor from "@coral-xyz/anchor";
import { buildRemoveCanMintMessage } from "./helpers";
import { initializeMultisigContext, buildAndSendMultisigTransaction, handleError } from "./shared-utils";

export async function removeCanMint(target: PublicKey): Promise<string> {
  console.log("\n=== Remove Can Mint Operation ===");
 ...
 // Set up event listener BEFORE building transaction
        ...
        const listenerId = context.program.addEventListener('blacklistedMinter', (event: any, slot: number) => {
            console.log("\n=== Event Received ===");
