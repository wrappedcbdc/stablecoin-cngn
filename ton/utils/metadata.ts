...
export function buildOnchainMetadata(data: { name: string; description: string; symbol: string; decimals: number; image: string }): Cell {
    ...
    Object.entries(data).forEach(([key, value]) => {
        ...
        dict.set(toKey(key), makeSnakeCell(Buffer.from(String(value), "utf8")));
    });
